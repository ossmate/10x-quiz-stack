Frontend - Astro with React for interactive components:

- Astro 5 enables the creation of fast, efficient websites and applications with minimal JavaScript
- React 19 provides interactivity where needed
- TypeScript 5 for static type checking and improved IDE support
- Tailwind 4 allows for convenient application styling
- Shadcn/ui offers a library of accessible React components for building the UI

Backend - Supabase as a comprehensive backend solution:

- Provides a PostgreSQL database
- Offers SDKs in multiple languages, serving as a Backend-as-a-Service
- It is an open source solution that can be self-hosted or hosted on your own server
- Includes built-in user authentication

AI - Communication with models via the Openrouter.ai service:

- Access to a wide range of models (OpenAI, Anthropic, Google, and many others) which allow us to find a solution that ensures high efficiency and low costs
- Allows setting financial limits on API keys

Testing:

- Vitest for unit and integration testing
- React Testing Library for component testing
- Playwright for end-to-end (E2E) testing
- Supertest for API endpoint testing
- axe DevTools and WAVE for accessibility testing

CI/CD and Hosting:

- Github Actions for creating CI/CD pipelines
- DigitalOcean for hosting the application via a Docker image
